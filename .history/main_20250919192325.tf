...
provider "aws" {
  ...
  # Фіктивні креденшіали для локальної перевірки синтаксису без реального підключення
...
# Приклад підключення модулів з умовними іменами ресурсів
...
# Kubernetes доступ (фіктивні дані під токен)
locals {
